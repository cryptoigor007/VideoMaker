# VideoMaker FIX | 2026.09.09-r43-tail | 2026-09-09
# CHANGED:
#   r43-tail: cut end = max(gemini_end, last_word_end) + 0.40s (clamp к video_duration)
#             чтобы последнее слово не обрезалось; лог [SHORTS] tail: ...
...
# Запас после последнего слова (сек), чтобы stream copy / Gemini end не съедали хвост речи.
_SHORT_TAIL_PAD = 0.40


def _last_speech_end(
    transcription: dict | None,
    clip_start: float,
    clip_end: float,
) -> float | None:
    """Абсолютный end последнего слова, пересекающего окно клипа.

    Без «чужих» слов далеко за границами: слово должно пересекать [clip_start, clip_end].
    """
    if not transcription or clip_end <= clip_start:
        return None
    ...
    for seg in transcription.get("segments") or []:
        ...
        if words:
            for w in words:
                ...
                if we <= ws:
                    we = ws + 0.12
                # пересечение с окном клипа
                if we > t0 + 0.02 and ws < t1 + 0.05:
                    last = we if last is None else max(last, we)
        else:
            # fallback: граница сегмента
            try:
                ss = float(seg.get("start", 0) or 0)
                se = float(seg.get("end", ss) or ss)
            except (TypeError, ValueError):
                continue
            if se > t0 + 0.02 and ss < t1 + 0.05:
                last = se if last is None else max(last, se)
    return last
...
class ShortsCutter(Stage):
    ...
    def _create_short(
        ...
        # Хвост речи: Gemini end часто раньше last_word / keyframe copy режет рано
        gemini_end = float(end)
        last_word = _last_speech_end(
            getattr(ctx, "transcription", None), start, gemini_end
        )
        if last_word is not None:
            target_end = max(gemini_end, float(last_word)) + _SHORT_TAIL_PAD
        else:
            target_end = gemini_end + _SHORT_TAIL_PAD
        target_end = min(float(video_duration), max(gemini_end, target_end))
        if target_end > start + 0.05:
            end = target_end
            duration = end - start
        ctx.log(
            f"[SHORTS] #{index} tail: gemini_end={gemini_end:.2f} "
            f"last_word={last_word if last_word is not None else 'n/a'} "
            f"pad={_SHORT_TAIL_PAD:.2f} → cut_end={end:.2f} (limit={video_duration:.2f})"
        )
        ...
        # stream copy иногда короче запрошенного — один VT-retry
        try:
            got = float(probe_duration(cut_path) or 0)
        except Exception:
            got = 0.0
        if got > 0.2 and got < duration - 0.15:
            ctx.log(
                f"[SHORTS] #{index} cut shortfall: got={got:.2f}s need={duration:.2f}s → VT re-trim"
            )
            try:
                os.remove(cut_path)
            except OSError:
